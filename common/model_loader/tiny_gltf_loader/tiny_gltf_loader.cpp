...
lib::Buffer<std::byte> createIndices(
    const tinygltf::Model& model, const tinygltf::Primitive& primitive, IndexType* indexType) {
...
lib::Buffer<glm::vec3> createTangents(
    IndexType indexType, std::span<const std::byte> indicesBytes,
...
void ProcessNode(const tinygltf::Model& model, const tinygltf::Node& node,
  ...
  for (const tinygltf::Primitive& primitive : model.meshes[node.mesh].primitives) {
    ...
    lib::SharedBuffer<glm::vec3> tangents =
        createTangents(indexType, indicesBytes, positions, texCoords);
    ...
    vertexDataList.emplace_back(
        std::move(positions), std::move(texCoords), std::move(normals), std::move(tangents),
        std::move(indicesBytes), indexType, currentTransform, std::move(diffuseTexture),
