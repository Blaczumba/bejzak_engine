...
constexpr std::array<uint16_t, 121> fromKeyToGlfw = {
  GLFW_KEY_UNKNOWN,
...
MouseKeyboardManagerGlfw::MouseKeyboardManagerGlfw(const std::shared_ptr<const WindowGlfw>& window)
  : _window(window) {}

bool MouseKeyboardManagerGlfw::isPressed(Keyboard::Key key) const {
  return glfwGetKey(static_cast<GLFWwindow*>(_window->getNativeHandler()),
                    Keyboard::fromKeyToGlfw[static_cast<uint16_t>(key)])
         == GLFW_PRESS;
}
...
void MouseKeyboardManagerGlfw::setKeyboardCallback(Keyboard::Callback callback) const {
  ...
  glfwSetKeyCallback(static_cast<GLFWwindow*>(_window->getNativeHandler()),
...
void MouseKeyboardManagerGlfw::setMouseMoveCallback(Mouse::MoveCallback callback) const {
  ...
  glfwSetCursorPosCallback(static_cast<GLFWwindow*>(_window->getNativeHandler()),
...
void MouseKeyboardManagerGlfw::absorbCursor() const {
  glfwSetInputMode(
      static_cast<GLFWwindow*>(_window->getNativeHandler()), GLFW_CURSOR, GLFW_CURSOR_DISABLED);
}

void MouseKeyboardManagerGlfw::freeCursor() const {
  glfwSetInputMode(
      static_cast<GLFWwindow*>(_window->getNativeHandler()), GLFW_CURSOR, GLFW_CURSOR_NORMAL);
}
