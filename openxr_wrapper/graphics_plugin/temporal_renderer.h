...
ErrorOr<Texture> createCubemap(
    const LogicalDevice& logicalDevice, VkCommandBuffer commandBuffer, VkBuffer stagingBuffer,
    const ImageDimensions& dimensions, VkFormat format, float samplerAnisotropy) {
...
private:
  ...
  Status createDescriptorSets() {
    ...
    _assetManager.loadImageCubemapAsync(_logicalDevice, TEXTURES_PATH "cubemap_yokohama_rgba.ktx");
    {
      ...
      ASSIGN_OR_RETURN(
          _textureCubemap,
          createCubemap(_logicalDevice, commandBuffer, imgData.stagingBuffer.getVkBuffer(),
                        imgData.imageDimensions, VK_FORMAT_R8G8B8A8_UNORM, maxSamplerAnisotropy));
    }
  }
};
