...
GraphicsPluginVulkan::GraphicsPluginVulkan(PFN_vkDebugUtilsMessengerCallbackEXT debugCallback) : _debugCallback(debugCallback) {}
...
ErrorOr<lib::Buffer<VkExtensionProperties>> GetAvailableInstanceExtensions(std::string_view layerName) {
  uint32_t count;
  ...
  CHECK_VKCMD(vkEnumerateInstanceExtensionProperties(layerName.data(),
                                                     &count,
                                                     available_extensions.data()));
  return available_extensions;
}

ErrorOr<Instance> createInstance(std::string_view engineName, std::span<const char* const> requiredExtensions, PFN_vkDebugUtilsMessengerCallbackEXT debugCallback, XrInstance xrInstance, XrSystemId systemId) {
  ...
  CHECK_XRCMD(xrGetInstanceProcAddr(xrInstance, "xrGetVulkanGraphicsRequirements2KHR",
  ...
  CHECK_XRCMD(pfn_get_vulkan_graphics_requirements_khr(xrInstance,
                                                       systemId,
                                                       &graphics_requirements));
  PFN_xrCreateVulkanInstanceKHR pfn_xr_create_vulkan_instance_khr = nullptr;
  CHECK_XRCMD(xrGetInstanceProcAddr(xrInstance, "xrCreateVulkanInstanceKHR",
  ...
  CHECK_XRCMD(pfn_xr_create_vulkan_instance_khr(xrInstance,
                                                &vkInstanceCreateInfoKhr,
                                                &vkInstance,
...
ErrorOr<std::unique_ptr<PhysicalDevice>> createPhysicalDevice(XrInstance xrInstance, XrSystemId systemId, Instance& instance) {
  ...
  CHECK_XRCMD(pfn_get_vulkan_graphics_device_khr(xrInstance,
...
Status GraphicsPluginVulkan::initialize(XrInstance xrInstance, XrSystemId systemId) {
  ...
  ASSIGN_OR_RETURN(_instance, createInstance(
      "VR BejzakEngine", extensions, _debugCallback, xrInstance, systemId));
#ifdef VALIDATION_LAYERS_ENABLED
