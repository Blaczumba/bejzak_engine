...
public:
  ...
  static ErrorOr<PrimaryCommandBuffer> create(
      const std::shared_ptr<const CommandPool>& commandPool);

  template <size_t COUNT>
  static ErrorOr<std::array<PrimaryCommandBuffer, COUNT>> create(
      const std::shared_ptr<const CommandPool>& commandPool);

  static ErrorOr<std::vector<PrimaryCommandBuffer>> create(
  ...
  VkResult submit(QueueType type, const VkSemaphore waitSemaphore,
...
public:
  ...
  static ErrorOr<SecondaryCommandBuffer> create(
      const std::shared_ptr<const CommandPool>& commandPool);

  template <size_t COUNT>
  static ErrorOr<std::array<SecondaryCommandBuffer, COUNT>> create(
      const std::shared_ptr<const CommandPool>& commandPool);

  static ErrorOr<std::vector<SecondaryCommandBuffer>> create(
      const std::shared_ptr<const CommandPool>& commandPool, uint32_t count);
...
std::array<CommandBufferType, COUNT> transformCommandBuffers(
...
template <size_t COUNT>
ErrorOr<std::array<PrimaryCommandBuffer, COUNT>> PrimaryCommandBuffer::create(
    const std::shared_ptr<const CommandPool>& commandPool) {
  std::array<VkCommandBuffer, COUNT> commandBuffers;
  if (VkResult result = createCommandBuffers(
...
template <size_t COUNT>
ErrorOr<std::array<SecondaryCommandBuffer, COUNT>> SecondaryCommandBuffer::create(
    const std::shared_ptr<const CommandPool>& commandPool) {
  std::array<VkCommandBuffer, COUNT> commandBuffers;
  if (VkResult result = createCommandBuffers(
          commandPool->getLogicalDevice().getVkDevice(), commandPool->getVkCommandPool(),
...
template <size_t COUNT>
ErrorOr<std::array<SecondaryCommandBuffer, COUNT>>
CommandPool::createSecondaryCommandBuffers() const {
