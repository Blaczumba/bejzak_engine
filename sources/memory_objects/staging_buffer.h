...
private:
    ...
    struct Allocator {
        ...
        lib::ErrorOr<BufferData> operator()(VmaWrapper& wrapper) {
            ...
            return BufferData{ buffer.buffer, buffer.allocation, buffer.mappedData };
        }
...
lib::ErrorOr<std::unique_ptr<StagingBuffer>> StagingBuffer::create(MemoryAllocator& memoryAllocator, const Buffer& buffer) {
    ...
    ASSIGN_OR_RETURN(const BufferData bufferData, std::visit(Allocator{ size }, memoryAllocator));
